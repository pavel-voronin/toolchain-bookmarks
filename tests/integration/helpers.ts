...
export function createRepoTarball(tmpRoot: string): string {
  ...
  execFileSync('tar', ['-czf', tarball, '-C', tmpRoot, 'toolchain-bookmarks-main']);
  return tarball;
}
...
export function runCmd(cmd: string, args: string[], cwd: string, env: NodeJS.ProcessEnv = {}): CmdResult {
...
export function writeBookmarksFixture(runDir: string, withOpenAi = false): void {
  const payload = withOpenAi
    ? {
        ...
        roots: {
          ...
          other: { id: '2', guid: 'g2', name: 'other', date_added: '1', type: 'folder', children: [] },
          synced: { id: '3', guid: 'g3', name: 'synced', date_added: '1', type: 'folder', children: [] }
        }
      }
    : {
        checksum: 'v1',
        ...
        roots: {
          bookmark_bar: {
            ...
            type: 'folder',
            children: [
              { id: '10', guid: 'g10', name: 'Inbox', date_added: '1', type: 'folder', children: [] }
            ]
          },
          other: { id: '2', guid: 'g2', name: 'other', date_added: '1', type: 'folder', children: [] },
          synced: { id: '3', guid: 'g3', name: 'synced', date_added: '1', type: 'folder', children: [] }
  ...
  fs.writeFileSync(path.join(runDir, 'bookmarks.json'), `${JSON.stringify(payload, null, 2)}\n`, 'utf8');
