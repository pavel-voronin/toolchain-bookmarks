...
import { createRepoTarball, createTempDir, runInstall } from './helpers';
...
describe('install', () => {
  test('installs binary and initializes runtime layout', () => {
    ...
    expect(fs.existsSync(path.join(runDir, 'extension', 'manifest.json'))).toBe(true);
    expect(fs.existsSync(path.join(runDir, 'skills', 'bookmarks', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(runDir, 'systemd', 'bookmarks-make-diff.service'))).toBe(true);
    expect(fs.existsSync(path.join(runDir, 'systemd', 'bookmarks-make-diff.timer'))).toBe(true);
