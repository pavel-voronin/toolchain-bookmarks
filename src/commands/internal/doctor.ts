...
async function runDoctor(options: { json?: boolean } = {}): Promise<void> {
    ...
    ? listFilesRecursive(paths.skillDir)
        ...
        .flatMap((file) => {
          ...
          return /\{\{[A-Z0-9_]+\}\}/.test(content)
            ? [path.relative(paths.skillDir, file)]
            : [];
        })
    : [];
  ...
  if (!skillExists) {
    checks.push({
      name: "skill",
      status: "FAIL",
      message: "missing ./skills/bookmarks directory",
    });
  } else if (unresolved.length > 0) {
    checks.push({
      name: "skill",
      status: "FAIL",
      message: `unresolved placeholders in: ${unresolved.join(", ")}`,
    });
  } else {
  ...
  checks.push(
    fs.existsSync(paths.requestsDir)
      ...
      : {
          name: "requests",
          status: "FAIL",
          message: "missing ./requests directory",
        },
  );
