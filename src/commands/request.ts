...
export function runRequest(descriptionParts: string[] | string, options: { json?: boolean } = {}): void {
  ...
  fs.writeFileSync(
    target,
    [`ts=${new Date().toISOString()}`, `cwd=${paths.cwd}`, `description=${description}`].join('\n') + '\n',
    'utf8'
  );

  printOutput({ ok: true, requestFile: target }, Boolean(options.json), `request logged: ${target}`);
}
